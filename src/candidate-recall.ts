...
export async function evaluateCandidateRecall(
  ...
  const etaRegretSeconds =
    trihexBest && bestDriver ? Math.max(0, trihexBest.finalScore - bestDriver.durationSeconds) : 0;
  const relativeEtaRegretPct =
    bestDriver && bestDriver.durationSeconds > 0
