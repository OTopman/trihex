 ...
 */
export function validateCellId(id: unknown): asserts id is TriHexId {
  if (typeof id !== 'bigint') {
 ...
 */
export function isValidCell(input: unknown): input is TriHexId {
  try {
    let id: TriHexId;
 ...
 */
export function validateResolution(resolution: number): void {
  if (
    typeof resolution !== 'number' ||
    !Number.isInteger(resolution) ||
    resolution < 0 ||
    resolution > BIT_LAYOUT.MAX_RESOLUTION
  ) {
    throw new RangeError(
 ...
 */
export function validateRadius(radius: number, maxRadius = 15): void {
