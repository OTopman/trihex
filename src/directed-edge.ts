import { getCellNeighbors } from './adjacency';
import { getHexDual, isDualCellId } from './hex-dual';
import { cellToBoundary } from './triangle-quadtree';
import {
  BIT_LAYOUT,
  EDGE_BIT_LAYOUT,
  GeoCoord,
 ...
 */
export function isDirectedEdge(id: unknown): id is TriHexEdgeId {
 ...
 */
export function validateDirectedEdge(edgeId: unknown): asserts edgeId is TriHexEdgeId {
 ...
 */
export function getDirectedEdge(origin: TriHexId, destination: TriHexId): TriHexEdgeId {
  validateCellId(origin);
 ...
 */
export function getDirectedEdgeOrigin(edgeId: TriHexEdgeId): TriHexId {
 ...
 */
export function getDirectedEdgeDestination(edgeId: TriHexEdgeId): TriHexId {
  validateDirectedEdge(edgeId);
  const origin = edgeId & EDGE_BIT_LAYOUT.ORIGIN_MASK;
  const edgeIndex = Number((edgeId >> EDGE_BIT_LAYOUT.EDGE_INDEX_SHIFT) & 0x7n);
  const isDual = ((origin >> 53n) & 1n) === 1n;
 ...
 */
export function getDirectedEdgeBoundary(edgeId: TriHexEdgeId): [GeoCoord, GeoCoord] {
  validateDirectedEdge(edgeId);
  const origin = edgeId & EDGE_BIT_LAYOUT.ORIGIN_MASK;
  const edgeIndex = Number((edgeId >> EDGE_BIT_LAYOUT.EDGE_INDEX_SHIFT) & 0x7n);
  const isDual = ((origin >> 53n) & 1n) === 1n;
 ...
 */
export function getDirectedEdgeDetails(edgeId: TriHexEdgeId): TriHexDirectedEdge {
  validateDirectedEdge(edgeId);
