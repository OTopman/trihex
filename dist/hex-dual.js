...
exports.MAX_HEX_RING_RADIUS = void 0;
...
function getHexNeighbors(id) {
    ...
    for (const [du, dv] of HEX_OFFSETS) {
        ...
        const vec = [qx / qLen, qy / qLen, qz / qLen];
        const proj = (0, icosahedron_1.projectToFace)(vec);
        const { morton: nMorton } = (0, triangle_quadtree_1.barycentricToMorton)(proj.u, proj.v, resolution);
        neighbors.push((0, triangle_quadtree_1.packTriHexId)(proj.face, resolution, nMorton));
    }
...
function hexRing(originId, radius) {
    ...
    if (radius <= 0)
        return [originId];
    if (radius > exports.MAX_HEX_RING_RADIUS) {
        throw new RangeError(`Radius ${radius} exceeds maximum permitted limit (${exports.MAX_HEX_RING_RADIUS}) to prevent event-loop starvation`);
    }
    const visited = new Set();
    visited.add(originId.toString());
    let currentRing = [originId];
    const allCells = [originId];
...
function getHexDualBoundary(id) {
    ...
    return neighbors.map((n) => (0, triangle_quadtree_1.cellToLatLng)(n));
}
